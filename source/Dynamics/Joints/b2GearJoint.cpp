...
: b2Joint(def)
{
	...
	b2float32 coordinate1, coordinate2;
...
void b2GearJoint::SolveVelocityConstraints(const b2TimeStep& step)
{
	...
	b2float32 Cdot = m_J.Compute(	b1->m_linearVelocity, b1->m_angularVelocity,
...
bool b2GearJoint::SolvePositionConstraints()
{
	...
	b2float32 C = m_constant - (coordinate1 + m_ratio * coordinate2);
...
b2float32 b2GearJoint::GetReactionTorque() const
{
	// TODO_ERIN not tested
	b2Vec2 r = b2Mul(m_body2->m_xf.R, m_localAnchor2 - m_body2->GetLocalCenter());
	b2Vec2 F = m_force * m_J.linear2;
	b2float32 T = m_force * m_J.angular2 - b2Cross(r, F);
	return T;
}

b2float32 b2GearJoint::GetRatio() const
{
	return m_ratio;
}
